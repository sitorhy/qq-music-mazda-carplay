package name.sitorhy.server.model.album.songs;

import java.util.ArrayList;

public class File{
    public String media_mid;
    public long size_24aac;
    public long size_48aac;
    public long size_96aac;
    public long size_192ogg;
    public long size_192aac;
    public long size_128mp3;
    public long size_320mp3;
    public long size_ape;
    public long size_flac;
    public long size_dts;
    public long size_try;
    public long try_begin;
    public long try_end;
    public String url;
    public long size_hires;
    public long hires_sample;
    public long hires_bitdepth;
    public long b_30s;
    public long e_30s;
    public long size_96ogg;
    public ArrayList<Object> size_360ra;
    public long size_dolby;
    public ArrayList<Long> size_new;
}
